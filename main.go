...
func main() {
	...
	examSvc := service.NewExamService(userRepo)
	examController := controller.NewExamController(examSvc)

	r := gin.Default()
	api.SetupRoutes(r, userController, gradeController, courseController, examController)
